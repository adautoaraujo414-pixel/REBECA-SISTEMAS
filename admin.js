...
router.get('/whatsapp/status', async (req, res) => {
  try {
    ...
    const status = await MetaWhatsApp.verificarStatus(empresaId);
...
router.get('/whatsapp/config', async (req, res) => {
  try {
    const empresaId = req.user?.empresa_id || 1;
    const config = await MetaWhatsApp.getEmpresaConfig(empresaId);
    
    // Não retornar token completo por segurança
    if (config && config.whatsapp_token) {
      config.whatsapp_token = '***configurado***';
    }
...
router.post('/whatsapp/config', async (req, res) => {
  try {
    const empresaId = req.user?.empresa_id || 1;
    const { phoneId, businessId, accessToken, numero, appId } = req.body;
    ...
    const result = await MetaWhatsApp.salvarConfig(empresaId, {
      phoneId,
      businessId,
      accessToken,
      numero,
    ...
    if (result) {
      ...
      const status = await MetaWhatsApp.verificarStatus(empresaId);
      res.json({ success: true, message: 'WhatsApp configurado!', status });
    } else {
...
/**
 * POST /api/admin/whatsapp/testar
...
router.post('/whatsapp/testar', async (req, res) => {
  try {
    ...
    const resultado = await MetaWhatsApp.enviarMensagem(
      empresaId,
      numero,
      '✅ Teste do UBMAX!\n\nSua integração com WhatsApp Business API está funcionando perfeitamente!\n\n_Rebeca - Sua assistente virtual_'
    );
...
/**
 * GET /api/admin/whatsapp/webhook-url
 * Retornar URL do webhook para configurar no Facebook
 */
router.get('/whatsapp/webhook-url', async (req, res) => {
  try {
    const baseUrl = process.env.BASE_URL || `https://${req.headers.host}`;
    const webhookUrl = `${baseUrl}/webhook`;
    const verifyToken = process.env.WHATSAPP_WEBHOOK_VERIFY_TOKEN || 'ubmax_webhook_token_2024';
    
    res.json({ 
      success: true, 
      webhookUrl,
      verifyToken,
      instrucoes: 'Configure esta URL e token no Facebook Developer Console'
    });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
...
const EvolutionAPI = require('../whatsapp/evolution');

/**
 * GET /api/admin/whatsapp/status-conexao
...
router.get('/whatsapp/status-conexao', async (req, res) => {
  try {
    ...
    const empresaResult = await query(
      'SELECT whatsapp_instancia, whatsapp_conectado, whatsapp_rebeca FROM empresas WHERE id = $1',
      [empresaId]
    );
    
    if (!empresaResult.rows.length) {
      return res.json({ conectado: false, error: 'Empresa não encontrada' });
    }
    
    const empresa = empresaResult.rows[0];
    
    // Se não tem instância, não está conectado
    if (!empresa.whatsapp_instancia) {
      return res.json({ conectado: false });
    }
    
    // Verificar status real na Evolution API
    try {
      const status = await EvolutionAPI.verificarStatus(empresa.whatsapp_instancia);
      
      if (status && status.state === 'open') {
        return res.json({ 
          conectado: true, 
          numero: empresa.whatsapp_rebeca || status.number,
          instancia: empresa.whatsapp_instancia
        });
      }
    } catch (e) {
      console.log('Erro ao verificar Evolution:', e.message);
    }
    
    res.json({ conectado: empresa.whatsapp_conectado || false, numero: empresa.whatsapp_rebeca });
...
/**
 * POST /api/admin/whatsapp/gerar-qrcode
...
router.post('/whatsapp/gerar-qrcode', async (req, res) => {
  try {
    ...
    // Nome da instância para esta empresa
    const instanciaNome = 'rebeca_empresa_' + empresaId;
    
    console.log('📱 Gerando QR Code para instância:', instanciaNome);
    
    // Verificar se já existe e está conectada
    try {
      const status = await EvolutionAPI.verificarStatus(instanciaNome);
      if (status && status.state === 'open') {
        return res.json({ 
          success: true, 
          conectado: true, 
          numero: status.number,
    ...
    try {
      await EvolutionAPI.criarInstancia(instanciaNome);
    } catch (e) {
      // Pode já existir, continuar
    }
    
    // Conectar e obter QR Code
    const resultado = await EvolutionAPI.conectar(instanciaNome);
    
    if (resultado && resultado.qrcode) {
      // Salvar instância no banco
      await query(
        'UPDATE empresas SET whatsapp_instancia = $1, atualizado_em = CURRENT_TIMESTAMP WHERE id = $2',
        [instanciaNome, empresaId]
      );
    ...
    } else if (resultado && resultado.state === 'open') {
      // Já está conectado
      res.json({ 
        success: true, 
        conectado: true,
...
router.post('/whatsapp/desconectar', async (req, res) => {
  try {
    ...
    // Buscar instância
    const empresaResult = await query(
      'SELECT whatsapp_instancia FROM empresas WHERE id = $1',
      [empresaId]
    );
    
    if (!empresaResult.rows.length || !empresaResult.rows[0].whatsapp_instancia) {
      return res.json({ success: true, message: 'Não há WhatsApp conectado' });
    }
    
    const instancia = empresaResult.rows[0].whatsapp_instancia;
    
    // Desconectar na Evolution API
    try {
      await EvolutionAPI.desconectar(instancia);
    } catch (e) {
    ...
    // Atualizar banco
    await query(
      'UPDATE empresas SET whatsapp_conectado = false, atualizado_em = CURRENT_TIMESTAMP WHERE id = $1',
      [empresaId]
    );
